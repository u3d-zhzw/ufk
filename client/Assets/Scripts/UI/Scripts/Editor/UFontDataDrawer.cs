...
// {
    ...
    public class UFontDataDrawer : PropertyDrawer
    {
        ...
        private SerializedProperty m_FontAlias;
        // private UFontData m_FontData = null;
        private UIConfig.FontField[] m_fontFields = null;
        private string[] m_aliases = null;
        private int m_choiceIndex = 0;
        private string m_curAlias = null;
        ...
        protected void Init(SerializedProperty property)
        {
            ...
            // 设置字库列表
            m_FontAlias = property.FindPropertyRelative("FontAlias");
            m_curAlias = m_FontAlias.stringValue;

            m_fontFields = UIConfig.Fonts;
            int len = m_fontFields.Length;
            m_aliases = new string[len];
            for (int i = 0; i < len; ++i)
            {
                string name = m_fontFields[i].Name;
                m_aliases[i] = name;
                if (string.IsNullOrEmpty(m_curAlias) == false && m_curAlias == name)
                {
                    m_choiceIndex = i;
                }
            }
